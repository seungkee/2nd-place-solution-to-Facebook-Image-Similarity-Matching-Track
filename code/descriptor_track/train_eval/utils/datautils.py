...
class GaussianBlur(object):
    ...
    """
    def gaussian_blur(self, image, sigma, im_size):
    ...
    def __init__(self, kernel_size, p=0.5, im_size=224):
    ...
    def __call__(self, img):
        with torch.no_grad():
            ...
            if np.random.uniform() < self.p:
                return self.gaussian_blur(img, sigma=np.random.uniform(0.2, 2), im_size = self.im_size)
            return img
