...
class TwoCropTransform:
    ...
    def __init__(self,basic_transform,transform,hparams):
        ...
        if hparams.allgray == 1:
        ...
        elif hparams.allgray == 2:
            self.basic_transform = transforms.Compose([
                transforms.Resize((im_size, im_size)),
                transforms.RandomGrayscale(p=1.0),
                transforms.RandomPerspective(distortion_scale=0.6,p=1.0),
                transforms.RandomHorizontalFlip(p=0.5),
                transforms.RandomRoation(degress=(0,360)),
            ...
            #to be done. rotation and brightness change. no scale change.
        else:
            self.basic_transform = transforms.Compose([
                transforms.Resize((im_size,im_size)),
                transforms.ToTensor(),
    ...
    def __call__(self,x,y):
        return [self.basic_transform(x),self.transform(y)]
        #return [self.basic_transform(x), self.transform(x)]

class OverlayImageOnRandomBackground(torch.nn.Module):
    def __init__(self,backgroundDir='/facebook/data/images/train1M/train',size=(224,224),scale=(0.08,1.0),ratio=(0.5,2.0), bgblack= 0):
        super().__init__()
        ...
        self.rotate45andcrop = A.Compose([
                        A.Resize(size[0],size[0]),
                        A.Rotate([45,45],border_mode=0,p=1),
                        A.CenterCrop(int((1+r2)*size[0]/(r2+2)), int((1+r2)*size[0]/(r2+2)),always_apply=True)
                        ])
    @staticmethod
    ...
    def forward(self,img):
        if self.bgblack == 0:
            background_path=random.choice(self.backgroundFiles)
        ...
        backImage.paste(im=img,box=(j,i))
        return backImage
...
class PathDataset(torch.utils.data.Dataset):
    def __init__(self,r_paths,q_paths,transform,hparams):
        self.r_paths=r_paths
        self.q_paths=q_paths
        self.transform=transform
        self.hparams=hparams
    def __len__(self):
        return len(self.q_paths)
    def __getitem__(self,index):
        with open(self.r_paths[index],'rb') as f:
            r_img=Image.open(f)
            r_img = r_img.convert('RGB')
        with open(self.q_paths[index],'rb') as f:
            q_img=Image.open(f)
            q_img=q_img.convert('RGB')
        if self.transform is not None:
            img=self.transform(r_img,q_img)
        return img
...
class BaseSSL(nn.Module):
    ...
    def prepare_data(self):
        ...
        if self.hparams.data == 'cifar':
            ...
            query_dir = '/facebook/data/images/query'
            a=pd.read_csv('/facebook/data/public_ground_truth.csv')
            a=a[:len(a)-self.hparams.valid_size].reset_index(drop=True)
            a = a[~a['reference_id'].isnull()]
            q_ids = list(a['query_id'])
            r_ids = list(a['reference_id'])
            q_paths = [os.path.join(query_dir,x+'.jpg') for x in q_ids]

            ref_dir = '/facebook/data/images/reference_1M_root/reference_1M/'
            r_paths = [os.path.join(ref_dir,x+'.jpg') for x in r_ids]

            self.trainset = PathDataset(q_paths, r_paths,transform=TwoCropTransform(basic_transform, train_transform, self.hparams),hparams=self.hparams)# q_transform =, r_transform = , self.hparams)
            self.testset = PathDataset(q_paths, r_paths, transform=TwoCropTransform(basic_transform, train_transform, self.hparams),hparams=self.hparams)
            """
...
class SimCLR(BaseSSL):
    ...
    def samplers(self):
        if self.hparams.dist == 'ddp':
            ...
            trainsampler = torch.utils.data.distributed.DistributedSampler(self.trainset,shuffle=True)# shuffle=False)
    ...
    def transforms(self):
        if self.hparams.data == 'cifar':
            ...
            if self.hparams.aug_ver == 0 :
                ...
                train_transform = CustomAug_queryref_10060241(self.hparams)
...
class CustomAug_10032013(object):
    def __init__(self,hparams):
        im_size=hparams.im_size
        self.hparams = hparams
        self.cropresize=transforms.Compose([
...
class CustomAug_queryref_10060241(object):
    def __init__(self,hparams):
        im_size=hparams.im_size
        self.hparams = hparams
        self.m1=transforms.Compose([
                            transforms.RandomHorizontalFlip(0.5),
                            transforms.RandomVerticalFlip(0.5),
                            transforms.RandomApply([transforms.RandomRotation((90,90))],p=0.5),
                            transforms.RandomApply([transforms.RandomChoice(
