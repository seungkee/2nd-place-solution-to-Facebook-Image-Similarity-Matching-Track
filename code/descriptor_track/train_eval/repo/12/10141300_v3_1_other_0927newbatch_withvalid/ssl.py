...
class OverlayImageOnRandomBackground(torch.nn.Module):
    def __init__(self,backgroundDir=os.environ.get('TRAINDIR'),size=(224,224),scale=(0.08,0.5),ratio=(0.5,2.0),opacity=(0.4,1.0),p_scale=(0.25,1.0),p_ratio=(0.5,2.0), bgblack=0):
        super().__init__()
        ...
        self.dirname = os.environ.get('TRAINDIR')
        self.samples = list(np.load(os.environ.get('TRAINIMLIST')))
        self.size=size
        self.scale=scale
        self.ratio=ratio
...
class BaseSSL(nn.Module):
    ...
    def prepare_data(self):
        ...
        if self.hparams.data == 'cifar':
            ...
            traindir = os.environ.get('TRAINDIR')#os.path.join(self.IMAGENET_PATH, 'train')
            valdir = os.environ.get('TRAINDIR') #os.path.join(self.IMAGENET_PATH, 'val')
