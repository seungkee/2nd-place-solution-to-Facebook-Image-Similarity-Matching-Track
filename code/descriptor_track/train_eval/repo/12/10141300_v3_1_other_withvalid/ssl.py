...
class OverlayImageOnRandomBackground(torch.nn.Module):
    def __init__(self,backgroundDir=os.environ.get('TRAINDIR'),size=(224,224),scale=(0.08,1.0),ratio=(0.5,2.0), bgblack= 0):
        super().__init__()
...
class AttachEmoji(torch.nn.Module):
    def __init__(self,emojiDir=os.environ.get('EMOJIDIR'),scale=(0.08,1.0),ratio=(0.5,2.0),opacity=(0.1,0.9),emoji_ver=0):
        super().__init__()
        #self.backgroundFiles=glob.glob(backgroundDir)#os.listdir(backgroundDir)
...
class OverlayText(torch.nn.Module):
    def __init__(self,font_dir=os.environ.get('FONTDIR'),text_len=(5,30),x_pos=(-0.5,0.7),y_pos=(0,0.7),font_size=(0.1,0.3)):
        super().__init__()
...
class PathDataset(torch.utils.data.Dataset):
    def __init__(self,paths,transform,hparams):
        ...
        self.sim = np.load(os.environ.get('SIMPATH'))
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        if self.hparams.dataset_ver==5:
            ...
            #path=self.paths[torch.load(f'/siim/sim_pt_256/{t}_sim256.pt')[index%batch_size]]
            t+=self.hparams.seed
...
class BaseSSL(nn.Module):
    ...
    def prepare_data(self):
        ...
        if self.hparams.data == 'cifar':
            ...
            traindir = os.environ.get('TRAINDIR')#'/facebook/data/images/train1M/train/'#os.path.join(self.IMAGENET_PATH, 'train')
            valdir = os.environ.get('TRAINDIR')#'/facebook/data/images/train1M/train/' #os.path.join(self.IMAGENET_PATH, 'val')
    ...
    def transforms(self):
        if self.hparams.data == 'cifar':
            ...
            if self.hparams.aug_ver == 0 :
                ...
                train_transform = CustomAug_10032013(self.hparams) 
