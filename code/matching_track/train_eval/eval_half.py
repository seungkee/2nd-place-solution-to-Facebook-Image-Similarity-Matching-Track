...
class FacebookDataset_match_half(torch.utils.data.Dataset):
    def __init__(self,transform_q=None,transform_r=None,submission_df=None,mode='left',ref_rotate=0):
    ...
    def __getitem__(self, index):
        with open(self.query_paths[index], 'rb') as f:
            ...
            if self.mode=='left':
            ...
            elif self.mode=='vertical_center':
                ...
                img_query=transforms.functional.rotate(img_query,90)
                img_query=transforms.functional.crop(img_query,0,img_query.size[0]//2,img_query.size[1],img_query.size[0]//2)
            elif self.mode=='horizontal_center':
                img_query=img_query.resize((min(img_query.size),min(img_query.size)))
                img_query=transforms.functional.rotate(img_query,90)
                img_query=transforms.functional.crop(img_query,0,img_query.size[0]//4,img_query.size[1],img_query.size[0]//2)
            elif self.mode=='whole_rotate':
                img_query=img_query.resize((min(img_query.size),min(img_query.size)))
                img_query=transforms.functional.rotate(img_query,90)
...
def valid_all(model,args):
    ...
    submission_df['score']=-1
    for ref_rotate in [0]:
        for mode in ['whole','whole_rotate','left','right','vertical_center','bottom','top','horizontal_center']:
            if args.allgray == 1:
            ...
            if args.rank == 0:
                ...
                submission_df['score']=[max(a,b) for a,b in zip(np.array(submission_df['score']),outputs)]
