...
class OverlayImageOnRandomBackground(torch.nn.Module):
    def __init__(self,doPartialCrop,p_circleMask=0,p_affine=0,size=(224,224),scale=(0.08,0.5),ratio=(0.5,2.0),opacity=(0.8,1.0),p_scale=(0.25,1.0),p_ratio=(0.5,2.0),bgblack=0):
        ...
        self.attach_emoji=AttachEmoji(p_bg=0.5,p_onlybg=0.0,p_circleMask = 0, p_affine=0, scale=(0.05,0.5), ratio=(0.5,2.0), opacity=(0.2,1.0), cropscale=(0.08,0.5), cropratio=(0.5,2.0))
    ...
    def forward(self,img,batch_index_list=None):
        ...
        if random.random()>0.5:
            backImage=self.attach_emoji(backImage)
        if random.random()>0.5:
            backImage = self.attach_text(backImage)
...
class OverlayRandomImageOnImage(torch.nn.Module):
    ...
    def forward(self,img,batch_index_list):
        ...
        with open(overlay_path,'rb') as f:
            ...
            if random.random()>0.5:
                overImage=self.attach_emoji(overImage)
            if random.random()>0.5:
                overImage = self.attach_text(overImage)
    ...
    def transforms(self):
        if self.hparams.data == 'cifar':
            ...
            if self.hparams.aug_ver == 0 :
                ...
                train_transform=CustomAug_10072229(self.hparams)
...
class CustomAug_10072229(object):
    def __init__(self, hparams):
        im_size=hparams.im_size
        self.im_size=im_size
        self.attach_emoji=AttachEmoji(p_bg=0.5,p_onlybg=0.0,p_circleMask = 0, p_affine=0, scale=(0.05,0.5),ratio=(0.5,2.0),opacity=(0.2,1.0),cropscale=(0.08,0.5),cropratio=(0.5,2.0))
        self.attach_text=OverlayText()
        self.emojitext=transforms.Compose([
                                transforms.RandomApply([self.attach_emoji],p=0.5),
                                transforms.RandomApply([self.attach_text],p=0.5)
                            ])
        ...
        self.rotate45andcrop = A.Compose([
                        ...
                        A.CenterCrop(int((1+r2)*im_size/(r2+2)), int((1+r2)*im_size/(r2+2)),always_apply=True)
                        ])
        self.afterrotate45andcrop = transforms.Compose([
                            transforms.Resize((im_size,im_size)),
                            transforms.RandomHorizontalFlip(0.5),
                            transforms.RandomVerticalFlip(0.5),
                            transforms.RandomApply([transforms.RandomRotation((90,90))],p=0.5),
    ...
    def __call__(self,image,batch_index_list):
        ...
        if r<1/4:
            if random.random()>0.5:
                ...
                image= Image.fromarray(self.rotate45andcrop(image=np.array(image))['image'])
                image = self.afterrotate45andcrop(image)
        elif r<2/4:
            image=self.attachimage(image,batch_index_list)
            image=self.afterattachimage(image)
        elif r<3/4:
            image = self.emojitext(image)
