...
class FacebookDataset_match(torch.utils.data.Dataset):
    def __init__(self,transform_q=None,transform_r=None, submission_df = None):
...
def valid_all(model, args):
    ...
    if args.allgray == 1:
        dataset = FacebookDataset_match(transform_q=gray_transform, transform_r=gray_transform, submission_df = submission_df)
    else:
        dataset = FacebookDataset_match(transform_q=transform, transform_r=transform, submission_df = submission_df)
    ...
    if args.rank == 0:
        ...
        submission_df.to_csv(args.total_df+'_withmatchscore.csv', index=False)
